...
def _run_delegate(task: str, profile: str = None) -> str:
    """
    创建子 Agent 执行任务，返回结果。
...
def _build_profile_description() -> str:
    """动态生成角色参数描述"""
    try:
        from tools.profile_manager import ProfileManager
        ...
        if profiles:
            ...
            for pn in profiles:
                try:
                    ...
                    d = ""
                descs.append(f"{pn}({d})" if d else pn)
            return "子 Agent 的角色: " + ", ".join(descs) + "。不指定则用默认助手。"
    except Exception:
        pass
    return "子 Agent 的角色。不指定则使用默认助手角色。"
...
register(
    ...
    handler=lambda task, profile=None, **kw: _run_delegate(task, profile),
)
