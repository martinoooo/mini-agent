...
from cron.kanban_scheduler import KanbanScheduler  # ← Step 35
...
class AIAgent:
    def __init__(
        ...
        # ── Kanban 调度器（Step 35）─────────────────
        self.kanban = KanbanScheduler(api_key, base_url, model)
        self.kanban.start()
        # ── Provider 抽象层 ──────────────────────────
        ...
        # ── 构建 system prompt（Profile + 记忆 + 技能）──
        if profile:
            from tools.profile_manager import ProfileManager
            p = ProfileManager.load(profile)
        ...
        # ── 可用角色列表 ────────────────────────────
        from tools.profile_manager import ProfileManager
        profile_names = ProfileManager.list_profiles()
        if profile_names and not profile:
            profile_list = []
            for pn in profile_names:
                try:
                    p = ProfileManager.load(pn)
                    desc = p.soul.split("\n")[0].lstrip("# ")[:40]
                except Exception:
                    desc = ""
                profile_list.append(f"@{pn}" + (f" — {desc}" if desc else ""))
            parts.append(
                f"[可用角色] {len(profile_names)} 个:\n"
                + "\n".join(f"  {pl}" for pl in profile_list) + "\n"
                "使用 delegate_task(profile='角色名') 委派任务给对应角色。\n"
                "使用 kanban_create(assignee='角色名') 在看板上给角色分配任务。"
            )
    ...
    def _refresh_profiles(self):
        ...
        try:
            ...
            profile_text = (
                f"[可用角色] {len(profile_names)} 个:\n"
                + "\n".join(f"  {pl}" for pl in profile_list) + "\n"
                "使用 delegate_task(profile='角色名') 委派任务。\n"
                "使用 kanban_create(assignee='角色名') 在看板分配任务。"
            )
            base = self.system_prompt.split("[可用角色]")[0].strip()
        ...
        while iteration < self.max_iterations:
            ...
            if finish_reason == "tool_calls" and tool_calls:
                ...
                if len(tool_calls) == 1:
                    ...
                    self.log.info("[%s] %s(%s)", iteration, name,
                                  redact(json.dumps(args, ensure_ascii=False)))
                    ...
                    with ThreadPoolExecutor(max_workers=min(len(tool_calls), 4)) as executor:
                        ...
                        for future in as_completed(futures):
                            ...
                            try:
                                ...
                                result = f"错误: {e}"
                            self.log.info("[%s] %s → %s", iteration,
                                          tc["function"]["name"], redact(result[:120]))
                    ...
                    for tc in tool_calls:
                        self.messages.append({
                            "role": "tool", "tool_call_id": tc["id"],
                            "content": redact(results.get(tc["id"], "错误: 未找到结果")),
                        })
                continue
